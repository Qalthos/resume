% LaTeX file for resume
% This file uses the resume document class (res.cls)

\documentclass[linemargin]{res}
\usepackage{hyperref}
\topmargin=-0.5in  % start text higher on the page
\setlength{\textheight}{10in} % increase text height to fit resume on 1 page
\begin{document}
\name{Nathaniel Case}

\begin{resume}

\section{OBJECTIVE}
    Seeking a full-time position.

\section{EDUCATION}
    Rochester Institute of Technology, Rochester, NY \\
    BS in Computer Science | G.P.A. 3.48/4.0

\section{LANGUAGES AND TECHNOLOGIES}
    Python: twisted, TurboGears2, SQLAlchemy, BeautifulSoup\\
    Web: HTML, JavaScript, jQuery, CSS\\
    Experience in Java, bash, SQL, and others

\section{OPEN SOURCE PROJECTS}
    \begin{format}
        \title{l}\dates{r}\\
        \employer{l}\\
        \body
    \end{format}

    \title{infoboard}
    \employer{\url{http://github.com/FOSSRIT/infoboard}}
    \dates{2012 - present}
    \location{Python, Twisted, GTK3, SQLAlchemy, Requests}
    \begin{position}
        Desktop app to track Github contributions in (near) real-time.
    \end{position}

    \title{Remysmoke}
    \employer{\url{http://github.com/Qalthos/remysmoke}}
    \dates{2011 - present}
    \location{Python, TurboGears2, ToscaWidgets2, OpenShift}
    \begin{position}
        A web tracker to watch smoking habits. \url{http://remysmoke.linkybook.com/}
    \end{position}

    \title{Election Dashboard}
    \employer{\url{http://github.com/FOSSRIT/monroe-elections}}
    \dates{2010 - present}
    \location{Python, BeautifulSoup, Twisted, HTML5, jQuery}
    \begin{position}
        A dashboard displaying live results from elections in NY. \url{http://foss.rit.edu/election/}
    \end{position}

    \title{GrooveBot}
    \employer{\url{http://github.com/FOSSRIT/groovebot}}
    \dates{2010 - present}
    \location{Python, Twisted}
    \begin{position}
        An IRC bot that takes song requests and plays them over shared speakers.
    \end{position}

    \title{CIVX}
    \employer{\url{http://github.com/civx/civx}}
    \dates{2010 - 2011}
    \location{Python, TurboGears2, Moksha, jQuery}
    \begin{position}
        A web platform for civic engagement and inspection. \url{http://civx.us/}
    \end{position}

\section{WORK EXPERIENCE}
    \begin{format}
        \title{l}\employer{r}\\
        \location{l}\dates{r}\\
        \body
    \end{format}

    \title{Lead Student Developer}
    \employer{Lab for Technological Literacy, RIT}
    \dates{June 2010 - Present}
    \location{Rochester, NY}
    \begin{position}
        Worked on many projects in software (some listed above), instructed
        people, and managed the physical space of the FOSSBox.
    \end{position}

    \title{Informatics Co-op}
    \employer{Strong Memorial Hospital}
    \dates{June 2009 - May 2010}
    \location{Rochester, NY}
    \begin{position}
        Worked with researchers to transfer legacy paper forms to Teleforms OCR
        forms, web-accessible forms through ColdFusion, or Java applets.
    \end{position}

    \title{Software Co-op}
    \employer{MOOG Inc.}
    \dates{August 2008 - November 2008}
    \location{East Aurora, NY}
    \begin{position}
        Maintained and deployed a variety of web applications using J2EE,
        Struts, AJAX, and SQL
    \end{position}

\section{HONORS}
    Dean's List: Spring, Fall 2007, Winter, Spring 2008, Spring 2010 \\
    National Honor Socierty 2006 \\
    BSA Eagle Scout Award 2006 \\
    TIME Magazine Person of the Year 2006 \\

\section{SOURCE CODE}
    Available at \url{http://github.com/Qalthos}\\

\end{resume}

\end{document}
